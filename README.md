...
- Empty comma-separated items in `Address`, `AllowedIPs`, and `DisallowedIPs` are ignored to match the current SDK route parser. Non-empty malformed routes are still rejected. Empty DNS list items remain invalid so the UI does not silently accept an unusable resolver configuration.
